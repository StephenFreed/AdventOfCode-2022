...
void PrintInfoForRound(std::unordered_map<int, int>& monkey_total_inspected);
...
int main() {
    ...
    //      } 
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    std::vector<std::unordered_map<std::string, std::deque<int> > > directions = {   
               {
    ...
    std::unordered_map<int, int> monkey_total_inspected;

    // number of rounds
    int num_of_rounds = 20;
    // number to devide by after multiplication
    int divid_number = 3;
    ...
    for (int i = 0; i < num_of_rounds; i++) {
        ...
        for (int j = 0; j < directions.size(); j++) {
            ...
            // list of items
            std::deque<int> current_items = current_map_info["starting_items"]; 
            for (int k = 0; k < current_items.size(); k++) {
                ...
                // get current item
                int current_item = current_items[k];
                
                // number after calculations
                int new_number;
                //add
                if (info_map_operation == Add) {
                    new_number = (current_item + info_map_operation_num) / divid_number;  // floor division
                }
                // multiply
                ...
                // squared
                    ...
                    new_number = (current_item * current_item) / divid_number;  // floor division
                }
                 
                // test if true
                if (new_number % info_map_test_num == 0) {
                    directions[info_map_true_monkey_idx]["starting_items"].push_back(new_number);
                } 
                else {
                    directions[info_map_false_monkey_idx]["starting_items"].push_back(new_number);
                }
            }
...
void PrintInfoForRound(std::unordered_map<int, int>& monkey_total_inspected) {
    ...
    int most_inspected = 0;
    int second_most_inspected = 0;
    ...
    int total_monkey_business = most_inspected * second_most_inspected;
