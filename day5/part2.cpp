    ...
    if (input_file.is_open()) {
        ...
        while(std::getline(input_file, current_line)) {
            ...
            if (from_stack >= 1 && from_stack <= 9 && to_stack >= 1 && to_stack <=9 ) {
                ...
                // build new vector to move all crates togethr to new stack | this is in reverse order from what we want
                std::vector<char> crates_to_move_reversed;
                for (int i = 0; i < num_of_moves; i++) {
                    // push to temp stack and pop
                    crates_to_move_reversed.push_back(new_from_stack.back());
                    new_from_stack.pop_back();
                }

                // add to new stack in reverse order | ending in all crates moved to new stack in original order
                for (int i = crates_to_move_reversed.size() - 1; i >= 0; i--) {
                    new_to_stack.push_back(crates_to_move_reversed[i]);
                }
            } 
            else {
